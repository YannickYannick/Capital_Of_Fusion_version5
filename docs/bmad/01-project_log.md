...
| [2026-08-23 18:14] Completed Task | Indicateur pause ? 2 lecteurs /explore | Outcome: piste utilisateur confirm?e (2 lecteurs YT superpos?s : principal + cycle/aftermovie). `onStateChange` insuffisant (un lecteur ? autoplay refus? reste ? non d?marr? ? sans ?v?nement) ? surveillance `getPlayerState()` toutes les 500 ms : visible seulement en lecture, relance sinon, buffering/fin ignor?s pour ?viter le clignotement. Filet 2500 ms supprim?. |
