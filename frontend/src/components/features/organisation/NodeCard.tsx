...
/**
 * Carte nœud pour l'annuaire /organisation/noeuds.
 * Image de couverture, type, nom, courte description → lien vers /organisation/noeuds/[slug].
 */
...
function nodeCoverUrl(node: OrganizationNodeApi): string | null {
...
export function NodeCard({ node }: { node: OrganizationNodeApi }) {
  const cover = nodeCoverUrl(node) || PLACEHOLDER_IMAGE;

  return (
    <Link
      href={`/organisation/noeuds/${node.slug}`}
    ...
    >
      <div className="relative aspect-[4/5]">
        ...
        <div className="absolute bottom-4 left-4 right-4">
          <span className="inline-block px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-widest bg-purple-600/80 text-white mb-2">
