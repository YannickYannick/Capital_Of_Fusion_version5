...
export default async function FestivalPlanningNavettesPage() {
  const t = await getTranslations("pages");
  const html = markdownToHtml(t("festivalPlanning.contentMarkdown"));

  return (
    <div>
      <p className="text-xs uppercase tracking-widest text-purple-300/90">
        {t("festivalPlanning.eyebrow")}
      </p>
      <h1 className="mt-3 text-4xl md:text-5xl font-extrabold tracking-tight">
        {t("festivalPlanning.title")}
      </h1>
      <p className="mt-4 text-white/60">{t("festivalPlanning.subtitle")}</p>

      {html ? (
        <div className={`mt-10 ${proseClasses}`} dangerouslySetInnerHTML={{ __html: html }} />
      ) : (
        <p className="mt-10 text-white/50">
          {t("festivalPlanning.empty")}
        </p>
      )}
    </div>
  );
